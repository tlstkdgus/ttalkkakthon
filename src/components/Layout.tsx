    ...
    <div
      className={cn(
        ...
        isSecret
          ...
          : 'boss-desktop font-gungsuh text-boss-ink',
      )}
    >
      <div
        className={cn(
      ...
      >
        {isSecret ? (
          ...
          <div className="win95-window flex flex-1 flex-col overflow-hidden">
            {/* XP 타이틀 바 */}
            <header
              className="flex shrink-0 items-center justify-between px-2 py-1.5 sm:px-3"
              style={{
                background:
            ...
            >
              ...
              <div className="flex items-center gap-0.5">
                {['–', '□', '✕'].map((s, i) => (
                  <span
                    key={i}
                    className="flex h-[18px] w-[18px] items-center justify-center text-[10px] font-bold leading-none sm:h-5 sm:w-5 sm:text-xs"
                    style={{
            ...
            {/* Win95 메뉴바 */}
            <div
              className="flex shrink-0 items-center border-b-2 border-b-[#808080] bg-boss-surface text-[11px] text-boss-ink select-none"
              role="menubar"
              aria-hidden
            >
              {['파일(F)', '편집(E)', '보기(V)', '즐겨찾기(A)', '도움말(H)'].map((item) => (
                <span
                  key={item}
                  className="cursor-default px-2.5 py-0.5 hover:bg-boss-primary hover:text-white"
                >
                  {item}
                </span>
              ))}
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-auto bg-boss-surface p-2 sm:p-3 md:gap-3 md:p-4">
              {/* 헤더 배너 */}
              <div className="win95-inset-white shrink-0">
                <div
                  className="border-b-2 border-b-[#808080] px-3 py-2 sm:px-4 sm:py-3"
                  style={{ background: 'linear-gradient(to right, #000080 0%, #1a1a8c 60%, #000080 100%)' }}
                >
                  <BrandTitle />
                </div>
                <div className="px-3 py-2 sm:px-4 sm:py-3">
                  <p className="text-xs leading-snug text-boss-muted sm:text-sm md:text-base">
                    {brand.tagline(mode)}
                  </p>
                </div>
              </div>
